...
def make_flow_movie_p(positive_event_previews, negative_event_previews, predicted_intensities, groundtruth_intensities,
                      predicted_aolps, groundtruth_aolps, predicted_dolps, groundtruth_dolps,
                      predicted_flows, groundtruth_flows):
    ...
    for i, flow in enumerate(predicted_flows):
        ...
        imgs = [positive_voxel.resize_(3, positive_voxel.shape[1] // 2, positive_voxel.shape[2] // 2),
                ...
                blank, blank,
                pred_intensity, pred_aolp, pred_dolp,
                pred_flow_rgb.float(),
                # pred_flow_rgb.float().resize_(3, pred_flow_rgb.shape[1] // 2, pred_flow_rgb.shape[2] // 2),
                gt_intensity, gt_aolp, gt_dolp]
        if groundtruth_flows:
            gt_flow = groundtruth_flows[i]
            gt_flow_rgb = flow2rgb(gt_flow[0, 0, :, :], gt_flow[0, 1, :, :], max_magnitude)
            imgs.append(gt_flow_rgb.float())
