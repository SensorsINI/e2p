...
class FAD_Head_mhy(nn.Module):
    def __init__(self, size):
        super(FAD_Head_mhy, self).__init__()
        ...
        # define base filters and learnable
        # 0 - 1/16 || 1/16 - 1/8 || 1/8 - 1 || 0 - 1
        low_filter = Filter(size, 0, size // 16, use_learnable=False)
        middle_filter = Filter(size, size // 16, size // 8, use_learnable=False)
        high_filter = Filter(size, size // 8, size, use_learnable=False)
...
def _mhy_test():
    ...
    img = Image.open('/home/mhy/firenet-pdavis/utils/fad/intensity.png').convert('L')
    img_var = Variable(img_transform(img).unsqueeze(0)).cuda()

    out = FAD_Head_mhy(320).cuda()(img_var)

    low = np.array(to_pil(out[:, 0:1, :, :].data.squeeze(0).cpu()))
    mid = np.array(to_pil(out[:, 1:2, :, :].data.squeeze(0).cpu()))
    high = np.array(to_pil(out[:, 2:3, :, :].data.squeeze(0).cpu()))
    print(low)

    Image.fromarray(low).save('/home/mhy/firenet-pdavis/utils/fad/low.png')
    Image.fromarray(mid).save('/home/mhy/firenet-pdavis/utils/fad/mid.png')
    Image.fromarray(high).save('/home/mhy/firenet-pdavis/utils/fad/high.png')
