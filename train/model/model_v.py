...
class V20(BaseModel):
    """
    Three branches for iad, sin/cos for AoLP output encoding (tobi)

    trainer.py must be modified to compute the new GT output sin cos targets.

    self.pred['a'] is modified to have 2 output channels. The reconstruction of AoLP in train/utils/render_e2p_output.py must use sin/cos outputs
    ...
    def __init__(self, num_bins=5, base_num_channels=32, kernel_size=3, num_recurrent_units=2, unet_kwargs={}):
        ...
        self.R1_a = ResidualBlock(base_num_channels)
        self.R2_a = ResidualBlock(base_num_channels)
        self.pred_a = nn.Conv2d(base_num_channels, 2, 3, 1, 1) # 2 output channels for S1/S2 of AoLP angle
