...
def normalize_image_sequence_(sequence, key='intensity'):
...
class Compose(object):
    ...
    def __call__(self, x, is_flow=False, is_polarization=False, is_aolp=False):
        for t in self.transforms:
            x = t(x, is_flow, is_polarization, is_aolp)
        return x
...
class CenterCrop(object):
    ...
    def __init__(self, size, preserve_mosaicing_pattern=True):
    ...
    def __call__(self, x, is_flow=False, is_polarization=False, is_aolp=False):
        """
            x: [C x H x W] Tensor to be cropped.
            is_flow: this parameter does not have any effect
            is_polarization: crop half size if is polarization
            is_aolp: this parameter does not have any effect
        Returns:
...
class RandomCrop(object):
    ...
    @staticmethod
    def get_params(x, output_size, preserve_mosaicing_pattern, is_polarization):
        w, h = x.shape[2], x.shape[1]
        if is_polarization:
            w = 2 * w
            h = 2 * h
    ...
    def __call__(self, x, is_flow=False, is_polarization=False, is_aolp=False):
        """
            x: [C x H x W] Tensor to be cropped.
            is_flow: this parameter does not have any effect
            is_polarization: crop half size if true
            is_aolp: this parameter does not have any effect
        Returns:
            Tensor: Cropped tensor.
        """
        i, j, h, w = self.get_params(x, self.size, self.preserve_mosaicing_pattern, is_polarization)
...
class RandomFlip(object):
    ...
    def __call__(self, x, is_flow=False, is_polarization=False, is_aolp=False):
        ...
        :param is_polarization: has no effect
        :param is_aolp: if True, do flip on each pixel except the masked region
        ...
        if is_aolp:
            if len(dims) == 1:
                flipped = torch.where(flipped == 0, flipped, 1 - flipped)
